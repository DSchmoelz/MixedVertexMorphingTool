...
from TestVMRigidBody import CreateTargetMesh, CreateDesignMesh
...
# filter_radius = 2
# blending_node_ids = []
# for node in InitialDesignMesh.Nodes:
#     if node.x >= -blending_x_min_max and node.x <= blending_x_min_max:
#         blending_node_ids.append(node)

# vm_blending_function = InitialDesignMesh.ComputeBlendingFunction(blending_node_ids, filter_radius)
# number_of_nodes = 2*2*(12)+1
# x_i = np.linspace(-12, 12, number_of_nodes)
# plot = Plot()
# plot.ax.plot(x_i, vm_blending_function)
# plt.tight_layout(pad=0.2)
# plt.show()
# plt.savefig(f"figx_{sub_folder}_blending.pdf")
...
# plot.ax.set_xticks(np.arange(-12, 12+2, step=2))
